...
function createMockDirectoryService(
	...
	return {
		...
		getGroupDetails: vi.fn(
			async (groupId: string) => groups.get(groupId) || null,
		),
...
describe("AccessService", () => {
	...
	beforeEach(() => {
		...
		groups.set(
			"group-um",
			createMockGroup("group-um", "authelia_user_manager"),
		);
		groups.set(
			"group-pm",
			createMockGroup("group-pm", "lldap_password_manager"),
		);
		groups.set("group-users", createMockGroup("group-users", "users"));
		groups.set(
			"group-developers",
			createMockGroup("group-developers", "developers"),
		);
	...
	describe("getUserPermissions", () => {
		...
		it("should return user_manager permissions for user_manager users", async () => {
			const permissions =
				await accessService.getUserPermissions("user_manager");
	...
	describe("isProtectedGroup", () => {
		it("should return true for protected groups", () => {
			...
			expect(accessService.isProtectedGroup("lldap_password_manager")).toBe(
				true,
			);
			expect(accessService.isProtectedGroup("authelia_user_manager")).toBe(
				true,
			);
		});
