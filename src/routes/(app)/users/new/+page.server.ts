...
export const load: PageServerLoad = async ({ locals }) => {
	...
	try {
		...
		return {
			error: m.user_permissions_check_failed({
	...
	create: async ({ request, locals }) => {
		...
		try {
			...
			if (firstName) {
				attributes.push({
					name: "first_name",
					values: [sanitizeString(firstName, 255)],
				});
			}
			if (lastName) {
				attributes.push({
					name: "last_name",
					values: [sanitizeString(lastName, 255)],
				});
			}
			...
			const passwordResult = await directoryService.changePassword(
				newUser.id,
				password,
			);
			if (!passwordResult.success) {
				...
				try {
					...
					console.error(
						"Failed to roll back user creation after password error:",
						deleteError,
					);
				}
				return fail(500, {
